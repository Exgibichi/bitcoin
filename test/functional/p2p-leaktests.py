...
Also test that nodes that send unsupported service bits to statusquod are disconnected
...
class CNodeNoVersionBan(CLazyNode):
    ...
    # NOTE: implementation-specific check here. Remove if statusquod ban behavior changes
    def on_open(self, conn):
        super().on_open(conn)
...
class P2PLeakTest(StatusquoTestFramework):
