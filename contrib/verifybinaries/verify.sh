...
###   statusquocore.org and statusquo.org and compares them.
...
WORKINGDIR="/tmp/statusquo_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://statusquocore.org"
HOST2="https://statusquo.org"
BASEDIR="/bin/"
VERSIONPREFIX="statusquo-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'statusquo-',
...
if [ $? -ne 0 ]; then
   echo "statusquo.org failed to provide signature file, but statusquocore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "statusquo.org and statusquocore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Statusquo Core binary release signing key installed?"
   fi
