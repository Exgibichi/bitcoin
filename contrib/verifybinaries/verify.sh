...
###   statusquo.cash and statusquo.org and compares them.
...
if [ $? -ne 0 ]; then
   echo "statusquo.org failed to provide signature file, but statusquo.cash did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "statusquo.org and statusquo.cash signature files were not equal?"
