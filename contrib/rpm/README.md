...
The RPM spec file provided here is for Statusquo-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://statusquo.cash/bin/ and
...
Currently only `Source10` is used, for the example `statusquo.conf` file.
...
in the statusquo git repository. At present only an SVG version of the Statusquo
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the statusquo
...
needed when 0.12.1 is released, a proper fix is already in the Statusquo git
master branch.
...
idea to build Statusquo Core against a static version of that library compiled
...
The `statusquo.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `statusquo-server` package. At this point, most RPM based
...
The `statusquo-server` package should have SELinux support. How to properly do
...
Statusquo-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/statusquo/). He has been
packaging Statusquo for Fedora at least since 2012.
...
4. I renamed the `statusquo` package that contains the Qt GUI to `statusquo-core` as
that appears to be how the general population refers to it, in contrast to
`statusquo-xt` or `statusquo-classic`. I wanted to make sure the general population
