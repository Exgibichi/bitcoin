...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CStatusquoAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/Exgibichi/statusquo/pull/10340, but that change has been
    // postponed until after https://github.com/Exgibichi/statusquo/pull/10244 to
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-statusquo-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CStatusquoAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CStatusquoAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CStatusquoAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CStatusquoAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CStatusquoAddress(address).ToString());
}
 ...
 * https://statusquotalk.org/?topic=54527, or
 * https://github.com/Exgibichi/statusquo/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CStatusquoAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CStatusquoAddress(dest).ToString(), key);
}
